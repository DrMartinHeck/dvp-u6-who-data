...
(df.sort_values(by=['Year', 'Country'])).head(10)
...
ax = plt.subplot()
sns.barplot(data = df, x = 'Country', y = 'GDP', log = True)
plt.show()
...
print('No, life expectancy variates much less than GDP.')
...
ax = sns.barplot(x = df.Country, y = df.GDP, hue = df.Year, log = True, palette = 'bright')
...
f, ax = plt.subplots(figsize=(10, 15)) 
sns.barplot(x = df.Country, y = df.LEABY, hue = df.Year, palette = 'bright', log = False)
plt.ylim(40, 85)
...
print('''Zimbabwe changes most, but is probably driven by death rate for young children.
2004 is the turning point.
On a logarithmic scale, China had the largest change. On a linear scale it is about a wash with the USA.
On a linear scale Zimbabwe changed least, but on the log scale, Mexiko seems to slightly underperform Germany on GDP growth
''')
...
print('Still China has the most growth and Zimbabwe the largest change in LEABY.')
...
g3 = sns.FacetGrid(df, col="Country", col_wrap=3, size=4, sharex = False)
...
plt.subplot(2,1,1)
ax = sns.pointplot(data = df, x = "Year", y = "GDP", hue = 'Country', markers = '+')
ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol = 6)
plt.yscale('log')
plt.subplot(2,1,2)
ax = sns.pointplot(data = df, x = "Year", y = "LEABY", hue = 'Country', markers = '+')
